...
set HLD_TempIntervals{ ly in HLD_HeatCascades, lc in HLD_LocationsOfLayer[ly]} ordered; 
...
param HLD_TI_tl{ly in HLD_HeatCascades, lc in HLD_LocationsOfLayer[ly], k in HLD_TempIntervals[ly,lc]};
param HLD_TI_tu{ly in HLD_HeatCascades, lc in HLD_LocationsOfLayer[ly], k in HLD_TempIntervals[ly,lc]};


set HLD_zones{ ly in HLD_HeatCascades, lc in HLD_LocationsOfLayer[ly]} ordered;

# lists temperature intervals belonging to each subnetwork
set HLD_SubNetworks{ ly in HLD_HeatCascades, lc in HLD_LocationsOfLayer[ly], z in HLD_zones[ly,lc]} ordered;
...
set HLD_Hk{ly in HLD_HeatCascades, lc in HLD_LocationsOfLayer[ly], k in HLD_TempIntervals[ly,lc]} := {s in HLD_Hot[ly,lc] : HLD_Streams_Tin[ly,s]>= HLD_TI_tu[ly,lc,k] and HLD_Streams_Tout[ly,s]< HLD_TI_tu[ly,lc,k]};
set HLD_Ck{ly in HLD_HeatCascades, lc in HLD_LocationsOfLayer[ly], k in HLD_TempIntervals[ly,lc]} := {s in HLD_Cold[ly,lc] : HLD_Streams_Tin[ly,s]<= HLD_TI_tl[ly,lc,k] and HLD_Streams_Tout[ly,s]> HLD_TI_tl[ly,lc,k]};
...
# Forbidden matches
set HLD_Forbidden_Matches{ly in HLD_HeatCascades, lc in HLD_LocationsOfLayer[ly]} within HLD_Matches[ly,lc] default {};
...
# Forbidden matches
set HLD_Forbidden_MatchesGroups{ly in HLD_HeatCascades, lc in HLD_LocationsOfLayer[ly], z in HLD_zones[ly,lc]} within HLD_MatchesGroups[ly,lc,z] default {};
...
#TODO  Heat requiered by cold stream j in interval k, TODO Calculate MCp by dh / dT  PROBLEM MAX MIN 
param HLD_Qcjk{ly in HLD_HeatCascades, lc in HLD_LocationsOfLayer[ly], k in HLD_TempIntervals[ly,lc], j in HLD_Cold[ly,lc]} :=
	if (HLD_Streams_Tin[ly,j] < HLD_TI_tu[ly,lc,k] and HLD_Streams_Tout[ly,j] > HLD_TI_tl[ly,lc,k]) then (
		HLD_Streams_dH[ly,j] * (min(HLD_Streams_Tout[ly,j],HLD_TI_tu[ly,lc,k]) - HLD_TI_tl[ly,lc,k])/ (HLD_Streams_Tout[ly,j] - HLD_Streams_Tin[ly,j])
	) else (
		0
	);
# Heat supplied by hot stream i in interval k
param HLD_Qhik{ly in HLD_HeatCascades, lc in HLD_LocationsOfLayer[ly], k in HLD_TempIntervals[ly,lc], i in HLD_Hot[ly,lc]} :=
	if (HLD_Streams_Tin[ly,i] > HLD_TI_tl[ly,lc,k] and HLD_Streams_Tout[ly,i] < HLD_TI_tu[ly,lc,k]) then (
		HLD_Streams_dH[ly,i] * (HLD_TI_tu[ly,lc,k] - max(HLD_Streams_Tout[ly,i],HLD_TI_tl[ly,lc,k]))/ (HLD_Streams_Tin[ly,i] - HLD_Streams_Tout[ly,i])
	) else (
		0
	);
...
param HLD_Qij_max{ly in HLD_HeatCascades, lc in HLD_LocationsOfLayer[ly], i in HLD_Hot[ly,lc], j in HLD_Cold[ly,lc]} default 
	min(sum{k in HLD_TempIntervals[ly,lc] } (HLD_Qcjk[ly,lc,k,j]*sum {u in HLD_UnitsOfStream[j]} HLD_Units_Fmax[u]), sum{k in HLD_TempIntervals[ly,lc] } (HLD_Qhik[ly,lc,k,i]*sum {u in HLD_UnitsOfStream[i]} HLD_Units_Fmax[u]));
...
#	Heat balance for hot streams in interval k
...
sum_penalty[ly,lc,z] = sum{u in HLD_Units} penalty_max[ly,lc,z,u] - sum{u in HLD_Units} penalty_min[ly,lc,z,u];
...
# Forbidden matches
subject to HLD_Forbidden_Matches_cstr{ly in HLD_HeatCascades, lc in HLD_LocationsOfLayer[ly], z in HLD_zones[ly,lc],(ig,jg) in HLD_Forbidden_MatchesGroups[ly,lc,z]} : 
	HLD_y_ijz[ly,lc,z,ig,jg] = 0;
